...
// FUNCTIONS
static void _Init(SSD13xx_InitTypeDef *SSD13xx_InitStructure,
        SSD13xx_handle_t *SSD13xx_Handle);
static void _PutChar(SSD13xx_handle_t SSD13xx_Handle, uint8_t pos_x,
        uint8_t pos_y, uint8_t ch);
static void _PrintF(SSD13xx_handle_t SSD13xx_Handle, uint8_t pos_x,
        uint8_t pos_y, const char* ptr_format, ...);
static void _Print(SSD13xx_handle_t SSD13xx_Handle, uint8_t pos_x,
        uint8_t pos_y, const char* ptr_str, uint8_t len);
static void _Clear(SSD13xx_handle_t SSD13xx_Handle);
static void _SetCursor(SSD13xx_handle_t SSD13xx_Handle, uint8_t Pos_X,
        uint8_t Pos_Y);
static inline void _WriteCommand(SSD13xx_handle_t SSD13xx_Handle,
        uint8_t Command);
static inline void _WriteData(SSD13xx_handle_t SSD13xx_Handle, uint8_t Data);

// SSD13xx类定义
struct _SSD13xx_Class SSD13xx = { .Init = _Init, .Clear = _Clear, .PrintF =
        _PrintF, .Print = _Print, .PutChar = _PutChar, .SetCursor = _SetCursor };

void _Init(SSD13xx_InitTypeDef *SSD13xx_InitStructure,
        SSD13xx_handle_t *SSD13xx_Handle) {
    // 创建句柄
    SSD13xx_handle_t ptr = (SSD13xx_handle_t) malloc(
            sizeof(SSD13xx_InitTypeDef));
...
void _PutChar(SSD13xx_handle_t SSD13xx_Handle, uint8_t pos_x, uint8_t pos_y,
        uint8_t ch) {
    uint8_t i;
    //设置光标位置在上半部分
...
void _PrintF(SSD13xx_handle_t SSD13xx_Handle, uint8_t pos_x, uint8_t pos_y,
...
void _Print(SSD13xx_handle_t SSD13xx_Handle, uint8_t pos_x, uint8_t pos_y,
        const char* ptr_str, uint8_t len) {
    uint8_t i = 0;
    for (i = 0; i < len; i++) {
        _PutChar(SSD13xx_Handle, pos_x + i, pos_y, ptr_str[i]);
    }
}
