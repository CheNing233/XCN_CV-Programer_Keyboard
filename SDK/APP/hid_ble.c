...
//    tmos_start_reload_task(hidEmuTaskId, START_REPORT_EVT, 10);
}
...
static void hidEmuStateCB(gapRole_States_t newState, gapRoleEvent_t *pEvent) {
    ...
    case GAPROLE_ADVERTISING:
        PRINT("Advertising..\n");
        Global_ProtoBLEConn = Board_Conn_Wait;
        break;

    case GAPROLE_CONNECTED: {
        Global_ProtoBLEConn = Board_Conn_OK;
    ...
    case GAPROLE_WAITING:
        Global_ProtoBLEConn = Board_Conn_Stop;
    ...
    case GAPROLE_ERROR:
        Global_ProtoBLEConn = Board_Conn_ERR;
...
static uint8_t hidEmuRcvReport(uint8_t len, uint8_t *pData) {
    ...
    if (Global_ProtoState == Board_BLEState) {
        if (pData[1] & 0x01) {
