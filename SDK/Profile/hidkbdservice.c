...
const uint8_t hidBootKeyInputUUID[ATT_BT_UUID_SIZE] = { LO_UINT16(
        BOOT_KEY_INPUT_UUID), HI_UINT16(BOOT_KEY_INPUT_UUID) };

// HID Boot Keyboard Output Report characteristic
const uint8_t hidBootKeyOutputUUID[ATT_BT_UUID_SIZE] = { LO_UINT16(
...
const uint8_t hidControlPointUUID[ATT_BT_UUID_SIZE] = { LO_UINT16(
...
const uint8_t hidProtocolModeUUID[ATT_BT_UUID_SIZE] = { LO_UINT16(
...
static uint8_t hidExtReportRefDesc[ATT_BT_UUID_SIZE] = { LO_UINT16(
...
static uint8_t hidReportLedOutProps = GATT_PROP_READ | GATT_PROP_WRITE
...
static uint8_t hidReportBootKeyOutProps = GATT_PROP_READ | GATT_PROP_WRITE
        | GATT_PROP_WRITE_NO_RSP;
...
bStatus_t Hid_AddService(void) {
    ...
    GATTServApp_InitCharCfg(INVALID_CONNHANDLE,
            hidReportConsumerInClientCharCfg);
    ...
    hidRptMap[4].cccdHandle =
            hidAttrTbl[HID_REPORT_CONSUMER_IN_CCCD_IDX].handle;
